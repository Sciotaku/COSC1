# Author: Rahul Gupta
# Date: 03/08/2023
# Purpose: Lab 4 EC: Cool Interfact
...
def draw_map():
    global start_vertex, goal_vertex, mouse_is_pressed, m_x, m_y, start_map, close_map, remove_vertex
    ...
    if not start_map:
        ...
        set_font_size(50)
        draw_text("Welcome to Dartmouth!", img_width//4, img_height//3)
        set_font_size(30)
        draw_text("Press s to access the campus map!", img_width//4, img_height//2.5)
        draw_text("Press e to exit the campus map!", img_width//4, img_height//2.20)
...
def press_key(key):
    global start_map, close_map, remove_vertex
    if key == "s":
        start_map = True
    if key == "e":
        close_map = True
